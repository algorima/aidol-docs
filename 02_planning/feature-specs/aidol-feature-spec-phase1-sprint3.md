...
멤버(AI 아이돌)가 사용자에게 먼저 보내는 첫 메시지의 생성 및 운영 방식에 대한 결정사항입니다.

| 항목 | 내용 |
|------|------|
| **생성 방식** | LLM 동적 생성 방식으로 유지 |
| **트리거 시점** | 그룹 생성 직후 채팅 첫 진입 시 자동 발화 |
| **발화 횟수** | 대화 이력이 없는 상태에서 1회만 발화 |
| **API 설계** | 선 발화 API 별도 분리 (프론트엔드에서 독립 호출, 방안 1 합의) |
| **개발 현황** | 프롬프트 설계 완료 / 백엔드 구현 진행 중 |
| **운영 방침** | 배포 후 빠르게 검증, 필요 시 유연하게 rollback 할 수 있도록 팀 내 준비 |
| **LLM 실패 시 fallback** | 고정 문구로 대체 (멤버 이름만 동적 치환) |
...
> 렌더링 참고: `\n\n` 기준 3개 단락이므로 4-6 렌더링 규칙에 따라 3개 말풍선으로 분리 표시됨
